...
Client.prototype.connect = function(callback, timeout) {
	...
	var doHandshake = function(data) {
		...
		var response = this.buff.toString('utf8', headerSize, headerSize + this.targetSize);
		...
		// Save overbuffered data
		var tmp = new Buffer(this.buff.length - headerSize - this.targetSize);
		this.buff.copy(tmp, 0, headerSize + this.targetSize);
		this.buff = tmp;
	...
	// TODO: Fix the ondata callback, and its functions (very messy)
